...
namespace HoloToolkit.UI.Keyboard
{
	...
	public class Keyboard : MonoBehaviour
	{
		...
		/// <summary>
		/// The distance the keyboard should be beneath the users gaze.
		/// </summary>
		[SerializeField]
		private float m_DistanceBeneathGaze = 0.05f;
        ...
        /// <summary>
        /// Focuser used for interaction with the keyboard.
        /// </summary>
		...
		private void Update()
		{
			...
			// Axis Slider
			bool updateSliderPos = m_Focuser.FocusHitInfo.gameObject == this.gameObject;
			if (updateSliderPos)
			{
				Vector3 relPos = this.transform.InverseTransformPoint(m_Focuser.FocusHitInfo.point);
		...
		public void PresentKeyboard(AFocuser focuser, string startText)
		{
			PresentKeyboard(focuser);
			Clear();
		...
		public void PresentKeyboard(AFocuser focuser, LayoutType keyboardType)
		{
			PresentKeyboard(focuser);
		...
		/// <param name="startText">The initial text to show in the Keyboard's InputField.</param>
		/// <param name="keyboardType">Specify the keyboard type.</param>
		public void PresentKeyboard(AFocuser focuser, string startText, LayoutType keyboardType)
		{
			PresentKeyboard(focuser, startText);
			ActivateSpecificKeyboard(keyboardType);
		}
        ...
        {
            float distance = (this.transform.position - FocusManager.Instance.GazeFocuser.TargetOrigin).magnitude;
        ...
        {
            this.transform.LookAt(m_Focuser.TargetOrigin);
