...
namespace HUX.Spatial
{
	...
	public class SolverSurfaceMagnetism : Solver
	{
		...
		{
            m_head = Veil.Instance.HeadTransform;
			...
			if (raycastMode == RaycastModeEnum.Box)
			{
				...
				{
					HUX.Utility.Raycast.DebugEnabled = true;
				}
			}
		...
		/// <summary>
		///   Wraps the raycast call in one spot.  Should add a tunable layer parameter and implement here.
		/// </summary>
		...
		private static bool DefaultRaycast(Vector3 origin, Vector3 direction, float distance, out HUX.Utility.RaycastResult result)
		{
			return HUX.Utility.Raycast.First(origin, direction, distance, HUX.Layers.ToMask(HUX.Layers.Surface), out result);
		}

		private static bool DefaultSpherecast(Vector3 origin, Vector3 direction, float radius, float distance, out HUX.Utility.RaycastResult result)
		{
			return HUX.Utility.Raycast.SphereFirst(origin, direction, radius, distance, HUX.Layers.ToMask(HUX.Layers.Surface), out result);
		}
		...
		Vector3 GetRaycastOrigin()
		{
            ...
            return m_head.transform.position;//Veil.Instance.HeadPositionFiltered;
		}
		...
		Vector3 GetRaycastEndPoint()
		{
			...
			if (raycastDirection == RaycastDirectionEnum.HeadFacing)
			{
				ret = m_head.transform.position/*Veil.Instance.HeadPositionFiltered*/ + m_head.forward;
			}
		...
		public override void SolverUpdate()
		{
			...
			{
				// Do the cast!
				HUX.Utility.RaycastResult result;
				bool bHit = DefaultRaycast(ray.origin, ray.direction, MaxDistance, out result);
				...
				if (bHit)
				{
					this.GoalPosition = result.Point + SurfaceNormalOffset * result.Normal + SurfaceRayOffset * ray.direction;
					this.GoalRotation = CalculateMagnetismOrientation(ray.direction, result.Normal);
				}
			}
			else if (raycastMode == RaycastModeEnum.Box)
			{
				...
				if (HUX.Utility.Raycast.CastBoxExtents(extents, transform.position, targetMatrix, ray, MaxDistance, DefaultRaycast, BoxRaysPerEdge, OrthoBoxCast, out positions, out normals, out hits))
				{
					Plane plane;
					float distance;
					...
					this.GoalPosition = ray.origin + ray.direction * Mathf.Max(CloseDistance, distance + SurfaceRayOffset + boxSurfaceOffset + verticalCorrectionOffset) + plane.normal * (0 * boxSurfaceOffset + SurfaceNormalOffset);
					this.GoalRotation = CalculateMagnetismOrientation(ray.direction, plane.normal);
					OnSurface = true;
				}
				else
				{
			...
			else if (raycastMode == RaycastModeEnum.Sphere)
			{
				...
				bool bHit = DefaultSpherecast(ray.origin, ray.direction, size, MaxDistance, out result);
				OnSurface = bHit;
		...
		{
			bool debugEnabled = HUX.Utility.Raycast.DebugEnabled;
