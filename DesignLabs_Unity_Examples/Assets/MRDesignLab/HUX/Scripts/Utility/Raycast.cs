...
namespace HUX.Utility
{
	public static class Raycast
	{
		...
		public delegate bool RaycastFunc(Vector3 origin, Vector3 direction, float distance, out RaycastResult result);

		public static bool First(Vector3 origin, Vector3 direction, float distance, LayerMask layerMask, out RaycastResult result)
		{
		...
		public static bool SphereFirst(Vector3 origin, Vector3 direction, float radius, float distance, LayerMask layerMask, out RaycastResult result)
		{
			result = default(RaycastResult);
		...
		public static List<RaycastResult> All(Vector3 origin, Vector3 direction, float distance, LayerMask layerMask)
		{
			return All(origin, direction, distance, layerMask, null);
		}

		public static List<RaycastResult> All(Vector3 origin, Vector3 direction, float distance, LayerMask layerMask, List<Collider> movedColliders)
		{
			// Check to see if the ray cast hits any of the requested Unity layers
			...
			if (layerMask != 0)
			{
				...
				if (raycastHits != null)
				{
					results = new List<RaycastResult>(raycastHits.Length);
					foreach (var raycastHit in raycastHits)
					{
						results.Add(new RaycastResult(raycastHit));
					}
				}
			}
				...
				foreach (var collider in movedColliders)
				{
					if ((collider.gameObject.layer & layerMask) != 0)
					{
						...
						if (collider.Raycast(ray, out hitInfo, distance))
						{
							if (colliderIndex >= 0)
							{
								results[colliderIndex] = new RaycastResult(hitInfo);
							}
							else
							{
								results.Add(new RaycastResult(hitInfo));
							}
						}
		...
		public static bool CastBoxExtents(Vector3 extents, Vector3 targetPosition, Matrix4x4 trs, Ray ray, float maxDistance, RaycastFunc raycastFunc, int raysPerEdge, bool ortho, out Vector3[] points, out Vector3[] normals, out bool[] hits)
		{
			...
			for (int x = -halfRaysPerEdge; x <= halfRaysPerEdge; x += 1)
			{
				for (int y = -halfRaysPerEdge; y <= halfRaysPerEdge; y += 1)
				{
					...
					hits[index] = raycastFunc(origin, direction, maxDistance, out rayHit);
	...
	{
		public RaycastResult(RaycastHit hit) : this(hit.collider, hit.point, hit.normal, hit.distance, hit.textureCoord, hit.textureCoord2)
		{
		}

		public RaycastResult(Collider collider, Vector3 point, Vector3 normal, float distance, Vector2 textureCoord, Vector2 textureCoord2)
		{
			this.layer = collider != null ? collider.gameObject.layer : Layers.Surface;

			if (this.layer == Layers.Surface)
			{
		...
		public static readonly RaycastResult None = default(RaycastResult);
	}
